...
LOSS_TYPE = "_LoRA_CE"  # must match your fine-tuning folder
...
def load_model_and_tokenizer() -> Tuple[AutoTokenizer, torch.nn.Module]:
    ...
    tokenizer.model_max_length = TOKENIZER_MAX_LEN  # enforce 512
    ...
    for i in tqdm(range(0, len(prompts), BATCH_SIZE), desc="Generating"):
        ...
        try:
            ...
            if "size of tensor" in str(e).lower():
                ...
                toks = tokenizer(batch, padding=True, truncation=True, max_length=shorter, return_tensors="pt").to(DEVICE)
                with torch.no_grad():
                    outputs = model.generate(
                        **toks,
                        max_new_tokens=MAX_NEW_TOKENS,
...
def evaluate_model(
    ...
    category_stats = {cat: {"total": 0, "refused": 0, "redirected": 0} for cat in set(categories) if cat != "safe"}
    ...
    rouge1 = getattr(rouge_scores["rouge1"], "mid", rouge_scores["rouge1"]).fmeasure
    rougeL = getattr(rouge_scores["rougeL"], "mid", rouge_scores["rougeL"]).fmeasure

    cm = confusion_matrix(y_true, y_pred, labels=["unsafe", "safe"])
    cat_metrics = {cat: {"% refused": 100 * v["refused"] / v["total"],
                         "% redirected": 100 * v["redirected"] / v["total"],
                         "total": v["total"]}
...
def main():
    ...
    print(f"Tokenizer max length = {TOKENIZER_MAX_LEN}, MAX_SOURCE_LENGTH = {MAX_SOURCE_LENGTH}, device = {DEVICE}")
